...
let pricesDirty=false;document.querySelector('#prices').addEventListener('input',()=>{pricesDirty=true});setInterval(()=>{if(!pricesDirty&&!document.querySelector('#prices input:focus')){localStorage.setItem('usage-quota-open',JSON.stringify(Array.from(document.querySelectorAll('.credential[open]')).map(d=>d.dataset.id)));location.reload()}},5000);
...
function buildCredentialGroups(){let panel=document.querySelector('.panel');if(!panel)return;let table=panel.querySelector('table');if(!table||!table.tBodies.length)return;let groups={};Array.from(table.tBodies[0].rows).forEach(r=>{let c=r.cells;if(c.length<9)return;let id=c[0].textContent.trim();(groups[id]??=[]).push(Array.from(c).slice(1).map(x=>x.textContent.trim()))});let open=new Set(JSON.parse(localStorage.getItem('usage-quota-open')||'[]'));let wrap=document.createElement('div');wrap.id='credentialGroups';wrap.innerHTML='<div class="group-tools"><input id="groupSearch" placeholder="搜索凭证"><button onclick="setGroups(true)">全部展开</button><button onclick="setGroups(false)">全部收起</button></div>';Object.entries(groups).sort((a,b)=>a[0].localeCompare(b[0])).forEach(([id,rows])=>{let d=document.createElement('details');d.className='credential';d.dataset.id=id;d.open=open.has(id);let sum=document.createElement('summary');sum.innerHTML='<strong>'+id+'</strong><span>'+rows.length+' 个模型</span><span class="chevron">⌄</span>';d.append(sum);let t=document.createElement('table');t.innerHTML='<thead><tr><th>模型</th><th>请求</th><th>普通输入</th><th>输出</th><th>缓存读</th><th>缓存写</th><th>命中率</th><th>费用</th></tr></thead><tbody>'+rows.map(x=>'<tr>'+x.map(v=>'<td>'+v+'</td>').join('')+'</tr>').join('')+'</tbody>';d.append(t);wrap.append(d)});table.replaceWith(wrap.querySelector('#credentialGroups')||wrap);let s=document.querySelector('#groupSearch');if(s)s.oninput=()=>{let q=s.value.toLowerCase();document.querySelectorAll('.credential').forEach(d=>d.hidden=!d.dataset.id.toLowerCase().includes(q))}}
function setGroups(open){document.querySelectorAll('.credential').forEach(d=>d.open=open);localStorage.setItem('usage-quota-open',JSON.stringify(open?Array.from(document.querySelectorAll('.credential')).map(d=>d.dataset.id):[]))}
document.head.insertAdjacentHTML('beforeend','<style>.group-tools{display:flex;gap:8px;margin-bottom:12px}.group-tools input{max-width:280px}.credential{border:1px solid var(--line);border-radius:12px;margin:10px 0;overflow:hidden;background:var(--card)}.credential summary{display:flex;gap:18px;align-items:center;padding:15px 18px;cursor:pointer;list-style:none;background:color-mix(in srgb,var(--accent) 5%,var(--card))}.credential summary::-webkit-details-marker{display:none}.credential summary strong{margin-right:auto}.credential summary span{color:var(--muted)}.credential summary .chevron{font-size:18px;transition:transform .15s}.credential[open] summary .chevron{transform:rotate(180deg)}.credential table{margin:0}.credential[hidden]{display:none}</style>');buildCredentialGroups();
</script></body></html>`))
